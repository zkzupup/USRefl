...
namespace Ubpa::detail 
{
  ...
  constexpr auto Acc(const L&, F&&, R r, std::index_sequence<>) { return r; }
...
namespace Ubpa::USRefl {
  ...
  struct ElemList 
  {
    ...
    { 
      Accumulate(0, [&](auto, const auto& field) {std::forward<F>(f)(field); return 0; });
    }
    ...
    { 
      return detail::FindIf(*this, std::forward<F>(f), std::make_index_sequence<size>{}); 
    }
    ...
    {
      return std::apply([e](auto...es){return ElemList<Es...,E>{es...,e};},elems);
    }
  ...
  struct TypeInfoBase 
  {
    ...
    static constexpr void ForEachVarOf(U&& obj, Func&& func) 
    {
      VirtualBases().ForEach(
        [&](auto vb) { 
          vb.fields.ForEach([&](const auto& fld){ using Fld = std::decay_t<decltype(fld)>;
